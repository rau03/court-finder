...
async function updateCourtMetadata() {
  const courtMetadata = {
    ...
    "Morley Field Pickleball Courts": {
      state: "CA",
      zipCode: "92104",
      indoor: false,
      numberOfCourts: 12,
  ...
  try {
    ...
    console.log(`Found ${courts.length} courts to update`);

    let updatedCount = 0;

    // Update each court with its metadata
    for (const court of courts) {
      if (courtMetadata[court.name]) {
        const metadata = courtMetadata[court.name];
        ...
        await Court.findByIdAndUpdate(
          court._id,
          {
            $set: {
              ...
              numberOfCourts: metadata.numberOfCourts,
            },
          },
          { new: true }
        );
    ...
    // Get the updated courts
    const updatedCourts = await Court.find({});

    return NextResponse.json({
      message: `Updated metadata for ${updatedCount} courts`,
      courts: updatedCourts,
    });
  } catch (error) {
    console.error("Error updating court metadata:", error);
    return NextResponse.json(
      {
        error: "Failed to update court metadata",
